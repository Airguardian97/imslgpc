...
from inventory.forms import StockForm
from django.http import JsonResponse
 # Make sure to import your Stock model here
...
def save_stock(request):
    if request.method == 'POST':
        ...
        if name:
            Stock.objects.create(name=name,quantity=0)
            return JsonResponse({'message': 'Data saved successfully'})
        else:
            return JsonResponse({'error': 'Stock name not provided'}, status=400)
    else:
    ...
    def post(self, request):
        ...
        if form.is_valid() and formset.is_valid():
            ...
            return redirect('sale-bill', billno=billobj.billno)
             ...
             messages.error(request, "Error: Please check your input and try again.")
...
def import_excel(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            sheet_name = request.POST.get('sheet_name')  # Get sheet name from form

            # Read Excel file into DataFrame
            df = pd.read_excel(excel_file, sheet_name=sheet_name,skiprows=15, usecols="A:H")
